...
class BraiinsPoolApiClient:
    ...
    async def get_account_stats(self):
        ...
        headers = {k: v.format(self._api_key) for k, v in API_HEADERS.items()} # Assuming API_HEADERS uses format strings
        try:
            ...
            _LOGGER.error("Error fetching account stats from Braiins Pool API: Status %s, %s", err.status, err.message)
            raise BraiinsPoolApiException(f"API error: Status {err.status}") from err
        except aiohttp.ClientError as err:
            _LOGGER.error("Error fetching account stats from Braiins Pool API: Network or client error: %s", err)
            raise err
    ...
    async def get_daily_rewards(self):
        ...
        try:
            async with self._session.get(daily_rewards_url, headers=headers) as response:
            ...
            if err.status == 401:
                _LOGGER.error("Braiins Pool API Authentication Error: Invalid API key.")
                raise BraiinsPoolAuthError("Invalid API key") from err
            _LOGGER.error("Error fetching daily rewards from Braiins Pool API: Status %s, %s", err.status, err.message)
            raise BraiinsPoolApiException(f"API error: Status {err.status}") from err
        except aiohttp.ClientError as err:
            _LOGGER.error("Error fetching daily rewards from Braiins Pool API: Network or client error: %s", err)
            raise err