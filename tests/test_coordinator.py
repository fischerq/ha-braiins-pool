...
async def test_successful_update_with_new_data(hass):
    ...
    async def mock_get_block_rewards(*args, **kwargs):
        return {"test_block_key": "block_rewards_data"} # Changed "test" to "test_block_key"
    mock_api_client.get_block_rewards = AsyncMock(side_effect=mock_get_block_rewards)

    async def mock_get_workers(*args, **kwargs):
        return {"test_worker_key": "workers_data"} # Changed "test" to "test_worker_key"
    mock_api_client.get_workers = AsyncMock(side_effect=mock_get_workers)

    async def mock_get_payouts(*args, **kwargs):
        return {"test_payout_key": "payouts_data"} # Changed "test" to "test_payout_key"
    mock_api_client.get_payouts = AsyncMock(side_effect=mock_get_payouts)
...
async def test_update_failed_parsing_error(hass):
    ...
    mock_api_client.get_user_profile = AsyncMock(side_effect=mock_get_user_profile_empty)
    ...
    async def mock_get_daily_hashrate(*args, **kwargs): return {"default_hash": "data"}
    mock_api_client.get_daily_hashrate = AsyncMock(side_effect=mock_get_daily_hashrate)
    async def mock_get_block_rewards(*args, **kwargs): return {"default_block": "data"}
    mock_api_client.get_block_rewards = AsyncMock(side_effect=mock_get_block_rewards)
    async def mock_get_workers(*args, **kwargs): return {"default_worker": "data"}
    mock_api_client.get_workers = AsyncMock(side_effect=mock_get_workers)
    async def mock_get_payouts(*args, **kwargs): return {"default_payout": "data"}
    mock_api_client.get_payouts = AsyncMock(side_effect=mock_get_payouts)
...
async def test_update_failed_daily_rewards_parsing_error(hass):
    ...
    mock_api_client.get_daily_rewards = AsyncMock(side_effect=mock_get_daily_rewards_malformed)

    async def mock_get_user_profile(*args, **kwargs):
        return {"btc": {"current_balance": 1.23, "all_time_reward": 10.0, "ok_workers": 1}}
    mock_api_client.get_user_profile = AsyncMock(side_effect=mock_get_user_profile)
    ...
    async def mock_get_block_rewards(*args, **kwargs): return {}
    mock_api_client.get_block_rewards = AsyncMock(side_effect=mock_get_block_rewards)
    async def mock_get_payouts(*args, **kwargs): return {}
    mock_api_client.get_payouts = AsyncMock(side_effect=mock_get_payouts)
    async def mock_get_workers(*args, **kwargs): return {}
    mock_api_client.get_workers = AsyncMock(side_effect=mock_get_workers)
...
async def test_update_failed_missing_new_data_keys(hass):
    ...
    async def mock_get_user_profile_missing_keys(*args, **kwargs):
        return {"btc": {"current_balance": 4.56}} # Missing 'all_time_reward', 'ok_workers'
    mock_api_client.get_user_profile = AsyncMock(side_effect=mock_get_user_profile_missing_keys)

    async def mock_get_daily_hashrate_empty_btc(*args, **kwargs):
        return {"btc": {}} # Will cause parsing error for pool_5m_hash_rate if that key is expected directly under "btc"
    mock_api_client.get_daily_hashrate = AsyncMock(side_effect=mock_get_daily_hashrate_empty_btc)

    async def mock_get_block_rewards(*args, **kwargs): return {"some_key": "some_value"}
    mock_api_client.get_block_rewards = AsyncMock(side_effect=mock_get_block_rewards)
    async def mock_get_workers(*args, **kwargs): return {"some_key": "some_value"}
    mock_api_client.get_workers = AsyncMock(side_effect=mock_get_workers)
    async def mock_get_payouts(*args, **kwargs): return {"some_key": "some_value"}
    mock_api_client.get_payouts = AsyncMock(side_effect=mock_get_payouts)
